...
class FallbackDirectoryResolverPlugin {
    constructor(options = {}) {
        ...
        console.warn('this.options.prefix', this.options.prefix)
    ...
    resolveComponentPath(reqPath) {
        if (!this.cache[reqPath]) {
            if (this.options.directories) {
                this.cache[reqPath] = Promise.filter(this.options.directories.map((dir) => path.resolve(path.resolve(dir), reqPath + '.tsx')), (item) => existsAsync(item).then((exists) => exists).catch(() => false)).any();
            }
            else {
